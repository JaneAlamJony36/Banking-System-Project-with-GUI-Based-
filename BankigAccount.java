...
public class BankigAccount {
    ...
    static boolean login(String role) {
        ...
        if (!isValidUsername(username)) {
            JOptionPane.showMessageDialog(null, "Invalid username. Must start with capital letter and contain only letters.");
            return false;
        }

        if (!isValidPassword(password)) {
            JOptionPane.showMessageDialog(null, "Invalid password. Must contain letter, number, symbol and be 8–32 characters long.");
            return false;
        }
